...
$powershell.AddScript({
  ...
  $Logfile = "D:\University work\Third Year\Final Year Project\Product\Logs\Master\PerformanceMonitor.log"
  ...
  if (Test-Connection $DbIP)
  {
    ...
    }#end set threshold for each host
    ...
    While($true) 
    {
      foreach($Influxhost in $Influxhosts)
      {
        if(Test-Connection $Influxhost.Name)
        {
        ...
        {
          write-log -Logfile $Logfile -Logstring "$($Influxhost.Name) unreachable resolve connectivity issues"
        }

      }#end for each influxhost
    }#end while loop
